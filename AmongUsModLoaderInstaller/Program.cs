...
namespace AmongUsModLoaderInstaller
{
    internal static class Program
    {
        ...
        private static async Task Install(bool server, bool steam, string gameDir, string runDir)
        {
            ...
            else
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    var url = JsonConvert.DeserializeObject<GithubRelease>(
                            await client.GetStringAsync("https://api.github.com/repos/BepInEx/BepInEx/releases/latest"))
                        .Url;

                    var prop = JsonConvert.DeserializeObject<AssetsProp>(await client.GetStringAsync(url));

                    var x86Url =
                        (from downloadableProp in prop.Assets
                            where downloadableProp.Name.Contains("x86")
                            select downloadableProp.Url).FirstOrDefault();

                    if (!gameDir.EndsWith("/")) gameDir += "/";

                    var zip = gameDir + "BepInEx.zip";
                    ...
                    else
                    {
                        ...
                        using (var fs = File.OpenWrite(zip))
                        {
        ...
        {
            [JsonProperty("assets")] public DownloadableProp[] Assets { get; set; }
        }
